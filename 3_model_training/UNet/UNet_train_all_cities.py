...
class ImageDataset(Dataset):
    def __init__(self, base_dir, cities, data_type):
        self.files = []
        for city in cities:
            naip_dir = os.path.join(base_dir, city, '1024', f'naip_{data_type}')
            roof_dir = os.path.join(base_dir, city, '1024', f'roof_{data_type}')
...
log_dir = f'runs/training_UNet_all_cities_1024_{current_time}'
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % 50 == 0:
        torch.save(model.state_dict(), rf'/workspace/ericyi/Surface Albedo/models/UNet/UNet_epoch_{epoch + 1}_all_cities_1024.pth')
