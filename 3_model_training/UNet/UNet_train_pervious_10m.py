...
#     def __init__(self, csv_file, sample_frac=0.1):
#         self.data_frame = pd.read_csv(csv_file).sample(frac=sample_frac, random_state=42).reset_index(drop=True)
#
#     def __len__(self):
#         return len(self.data_frame)
#
#     def __getitem__(self, idx):
#         naip_path = self.data_frame.iloc[idx, 0]
#         albedo_path = self.data_frame.iloc[idx, 1]
#
#         with rasterio.open(naip_path) as src:
#             naip_image = src.read(out_dtype='float32')
#
#         with rasterio.open(albedo_path) as src:
#             roof_image = src.read(1, out_dtype='float32')
#
...
#                torch.tensor(masked_roof_image[np.newaxis, ...], dtype=torch.float32)
#
# train_dataset = ImageDataset('/workspace/ericyi/Surface Albedo/data/train_files.csv', sample_frac=0.025)
# val_dataset = ImageDataset('/workspace/ericyi/Surface Albedo/data/test_files.csv', sample_frac=0.025)


# train_dir_naip = r'/workspace/ericyi/Surface Albedo/data/pervious_train'
# train_dir_roof = r'/workspace/ericyi/Surface Albedo/data/albedo_train'
#
#
# val_dir_naip = r'/workspace/ericyi/Surface Albedo/data/pervious_test'
# val_dir_roof = r'/workspace/ericyi/Surface Albedo/data/albedo_test'
#
class ImageDataset(Dataset):
...
log_dir = f'runs/training_UNet_pervious_1024_{current_time}'
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % 20 == 0:
        torch.save(model.state_dict(), rf'/workspace/ericyi/Surface Albedo/models/UNet/UNet_epoch_{epoch + 1}_pervious_1024.pth')
